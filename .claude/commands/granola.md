...
1. List recent Granola meetings. If $ARGUMENTS gives a date, count, or search term, use it to scope the list. Otherwise default to meetings from the last 7 days — run `date '+%Y-%m-%d'` in the shell to anchor that window; never infer today's date from session context.
